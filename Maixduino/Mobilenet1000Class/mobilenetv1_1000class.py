...
def main(labels = None, model_addr="/sd/m.kmodel", lcd_rotation=0, sensor_hmirror=False, sensor_vflip=False):
    ...
    try:
        while(True):
            ...
            img.draw_string(0, 120, "t:%dms" %(t), scale=2, color=(255, 0, 0))
            lcd.display(img)
...
if __name__ == "__main__":
    try:
        with open("labels.txt") as f:
            labels = f.readlines()
        main(labels=labels, model_addr="/sd/m.kmodel", lcd_rotation=0, sensor_hmirror=False, sensor_vflip=False)
